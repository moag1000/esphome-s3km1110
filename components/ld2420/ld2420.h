...
class LD2420Component : public Component, public uart::UARTDevice {
 public:
  ...
  // How many gates have no usable threshold.
  //
  ...
  // and nothing else in the output says so: calibration reports success, every
  // entity looks plausible, and presence simply never clears.
  //
  // Counted from the configuration actually in force rather than from the last
  // calibration run, so it survives a reboot and still describes the module
  // that is running. A counter of the run would read zero after any restart
  // while the capped thresholds were still loaded — the reassuring answer, and
  // the wrong one.
  uint8_t get_saturated_gate_count() const {
    uint8_t n = 0;
    for (uint8_t gate = 0; gate < TOTAL_GATES; gate++) {
      if (this->current_config.move_thresh[gate] >= 65535) {
        n++;
      }
    }
    return n;
