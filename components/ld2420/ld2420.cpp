...
// Deferred diagnostic message — logged in loop() after API connects
static char setup_diag_[256] = {0};
static bool setup_diag_logged_ = false;
...
static int get_firmware_int(const char *version_string) {
  ...
  if (version_str.empty() || !std::isdigit(static_cast<unsigned char>(version_str[0]))) {
    return 0;  // Non-numeric version string (e.g. "FAIL")
  }
  int version_integer = stoi(version_str);
  return version_integer;
}

float LD2420Component::get_setup_priority() const { return setup_priority::DATA; }  // After BUS (UART)
...
void LD2420Component::setup() {
  ...
  // corrupts the command/response framing in readline_().
  this->buffer_pos_ = 0;
  memset(this->buffer_data_, 0, sizeof(this->buffer_data_));
  this->cmd_active_ = false;
  this->cmd_reply_.ack = false;

  // Give the sensor time to settle after ESP32 reboot.
  // On warm boot the TX line may have briefly floated LOW (UART break),
  // which can put the sensor in an unresponsive state for several hundred ms.
  delay(500);

  // Drain buffered bytes and capture first 32 for protocol diagnosis.
  uint8_t drain_buf[32];
  int drained = 0;
  int captured = 0;
  while (this->available() && drained < 4096) {
    uint8_t b;
    this->read_byte(&b);
    if (captured < 32) {
      drain_buf[captured++] = b;
    }
    drained++;
  }

  // Try to enter config mode. On cold boot this succeeds immediately.
  // On warm boot (soft reboot), the sensor may not respond due to UART
  // glitches during GPIO reconfiguration. In that case, schedule a retry.
  if (this->set_config_mode(true) == LD2420_ERROR_TIMEOUT) {
    int pos = snprintf(setup_diag_, sizeof(setup_diag_),
                       "setup: config_mode timeout (drained=%d), scheduling retry", drained);
    // Schedule deferred retry — the sensor may need several seconds to recover
    this->set_timeout(5000, [this]() { this->retry_setup_(); });
    return;
  }
...
void LD2420Component::retry_setup_() {
  ESP_LOGW(TAG, "Retrying sensor initialization...");

  // Drain any accumulated bytes
  this->buffer_pos_ = 0;
  ...
  // Get UART hardware port number for ESP-IDF direct access
  auto *uart_comp = static_cast<esphome::uart::IDFUARTComponent *>(this->parent_);
  uart_port_t uart_num = static_cast<uart_port_t>(uart_comp->get_hw_serial_number());

  // Full UART recovery sequence:
  // 1. Delete UART driver (GPIO17 goes to input/floating = LOW = break)
  // 2. Wait 100ms for break to register on sensor
  // 3. Reconfigure GPIO17 as output HIGH (idle)
  // 4. Wait 100ms for sensor to see idle line
  // 5. Reinstall UART driver
  ESP_LOGW(TAG, "Performing full UART reset cycle on port %d...", uart_num);
  ...
  // Send CMD_RESTART (fire-and-forget) to try to wake the sensor.
  this->ld2420_restart();

  // Wait 1.5s for the sensor to restart, then try config mode.
  this->set_timeout(1500, [this]() {
    // Drain post-restart bytes
    this->buffer_pos_ = 0;
    int post_avail = 0;
    while (this->available()) {
      uint8_t b;
    ...
    static int retry_count = 0;  // shadows outer — reset on success
    retry_count++;

    ESP_LOGW(TAG, "Retry %d: drained %d bytes post-restart, trying config mode...",
             retry_count, post_avail);
    ...
    ESP_LOGW(TAG, "Sensor initialization succeeded on retry %d!", retry_count);
    setup_diag_[0] = 0;
    retry_count = 0;
    this->get_min_max_distances_timeout_();
#ifdef USE_NUMBER
...
void LD2420Component::loop() {
  // Output deferred setup diagnostics once (after API is connected)
  if (!setup_diag_logged_ && setup_diag_[0] != 0) {
    setup_diag_logged_ = true;
    ESP_LOGW(TAG, "%s", setup_diag_);
  }
...
void LD2420Component::readline_(int rx_data, uint8_t *buffer, int len) {
  ...
  // When waiting for a command response and a CMD_FRAME_HEADER is detected mid-stream,
  // realign the buffer so the header starts at position 0. This handles the case where
  // the sensor was streaming data (simple/energy mode) when a command was sent, and
  // the response header appears after some streaming bytes.
  if (this->cmd_active_ && this->buffer_pos_ >= 4) {
    // Check if the last 4 bytes are CMD_FRAME_HEADER and they don't start at position 0
    if (memcmp(&buffer[this->buffer_pos_ - 4], &CMD_FRAME_HEADER, sizeof(CMD_FRAME_HEADER)) == 0 &&
        this->buffer_pos_ > 4) {
      // Realign: move the header to position 0
      buffer[0] = buffer[this->buffer_pos_ - 4];
      buffer[1] = buffer[this->buffer_pos_ - 3];
      buffer[2] = buffer[this->buffer_pos_ - 2];
      buffer[3] = buffer[this->buffer_pos_ - 1];
      this->buffer_pos_ = 4;
      return;  // Continue collecting the rest of the response
    }
  }
...
int LD2420Component::send_cmd_from_array(CmdFrameT frame) {
  ...
  // Reset buffer state to prevent partial streaming data from corrupting response parsing.
  this->buffer_pos_ = 0;
  this->cmd_reply_.ack = false;
