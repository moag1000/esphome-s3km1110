...
esp_err_t uart_bridge_process_command(const char *json_line)
{
    ...
    } else if (strncmp(cmd.cmd, "ota_", 4) == 0) {
        /* OTA commands: ota_start, ota_data, ota_end, ota_abort */
        cJSON *root = cJSON_Parse(json_line);
        if (root == NULL) {
            char *resp = uart_proto_build_response(cmd.id, "error", "json parse error");
            send_and_free(resp);
        } else {
            ret = uart_ota_handle_command(cmd.cmd, root);
            cJSON_Delete(root);
            /* OTA module sends its own responses, no need to send here */
            if (ret != ESP_OK) {
...
void uart_bridge_rx_task(void *pvParameters)
{
    ...
    while (1) {
        /* Check OTA timeout periodically */
        uart_ota_check_timeout();
