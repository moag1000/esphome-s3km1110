...
static void esp_zb_task(void *pvParameters)
{
    ...
    if (ext_pan_id_is_zero) {
        /* Extended PAN ID not set - use IEEE MAC address via HAL */
        esp_err_t mac_ret = zb_hal_read_mac(extended_pan_id);
        if (mac_ret != ESP_OK) {
...
esp_err_t zb_coordinator_get_version(zb_coordinator_version_t *version)
{
    ...
    /* Coordinator type - from HAL */
    snprintf(version->type, sizeof(version->type), "%s", zb_hal_get_chip_name());
    ...
    /* Coordinator IEEE address via HAL */
    esp_zb_ieee_addr_t ieee_addr;
    zb_hal_read_mac(ieee_addr);
