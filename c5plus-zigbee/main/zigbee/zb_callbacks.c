...
/* LED status integration (optional) */
                              ...
                              esp_zb_ieee_addr_t ieee_addr)
{
    ...
    led_blink_notify(LED_NOTIFY_DEVICE_NEW);
}
...
void zb_callback_device_announce(esp_zb_zdo_signal_device_annce_params_t *device_annce)
{
    ...
    if (is_new_device) {
        ...
        if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
            ...
            led_blink_notify(LED_NOTIFY_DEVICE_FAILED);
        }
    } else {
        ESP_LOGI(TAG, "Device 0x%04X rejoined (existing device)", short_addr);
        led_blink_notify(LED_NOTIFY_DEVICE_KNOWN);
...
void zb_callback_permit_join_changed(uint8_t permit_duration)
{
    ...
    if (permit_join) {
        led_set_status(LED_STATUS_PAIRING);
    } else {
